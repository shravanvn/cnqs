...
cnqs::Nqs::Nqs(const Config &config) {
...
void cnqs::Nqs::RandInit(std::mt19937 &rng) {
...
void cnqs::Nqs::UpdateVars(const std::vector<double> &vars_diff) {
...
double cnqs::Nqs::VisibleBiasNorm() const {
...
double cnqs::Nqs::HiddenBiasNorm() const {
    double norm_squared = 0.0;
    for (int i = 0; i < 2 * h_; ++i) {
...
void cnqs::Nqs::Output(const std::string &file_name) const {
...
cnqs::Nqs cnqs::Nqs::ProposeUpdate(const Config &config,
                                   std::mt19937 &rng) const {
    cnqs::Nqs nqs_new(*this);
...
const double *cnqs::Nqs::WeightData() const {
